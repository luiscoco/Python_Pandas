...
```
pip install pandas matplotlib seaborn statsmodels tkinter
```
...
def select_column_and_decompose():
    ...
    if column in df.columns:
        ...
        decomposition = seasonal_decompose(time_series, model='additive', period=int(period_entry.get()))
...
correlation_button = tk.Button(root, text="Plot Correlation Matrix", command=plot_correlation_matrix)
...
decompose_button = tk.Button(root, text="Decompose Time Series", command=select_column_and_decompose)
